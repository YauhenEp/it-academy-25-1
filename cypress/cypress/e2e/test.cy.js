...
describe('test login', () => {
  ...
  it('should go to main page with valid credentials', () => {
    ...
    cy.validateText(header.userDropdownName, 'FirstnametestJohn LastnameTestDoe');
  })

  it('should be error notifications with invalid login', () => {
    loginPage.navigate('https://opensource-demo.orangehrmlive.com/')
    loginPage.login('sdfd','admin123')
    cy.get('.oxd-alert-content > .oxd-text').invoke('text').should('equal', 'Invalid credentials')
  })
  ...
  it('should be required notifications with empty login field', () => {
    ...
    cy.xpath(`//input[@name='username']/../following-sibling::span`).invoke('text').should('equal', 'Required')
  })
});